...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    ...
    [setOpenProp, open, setIsOpen],
  );

  // Helper to toggle the sidebar.
  const handleToggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile((open) => !open) : toggleSidebar();
...
function SidebarResizeHandle() {
  ...
  const { handleMouseDown: actualHandleMouseDown } = useSidebarResize({
    width,
    setWidth,
    minWidth,
    maxWidth,
  ...
  return (
    <div
      className={cn(
        "absolute top-0 right-0 w-1 h-full cursor-col-resize bg-transparent hover:bg-border/50 transition-colors z-20",
        "after:absolute after:inset-y-0 after:-left-1 after:-right-1 after:content-['']",
        isResizing && "bg-border",
      )}
      onMouseDown={actualHandleMouseDown}
      style={{ right: "-2px" }}
    />
  );
}
...
function Sidebar({
  ...
  const { isMobile, state, openMobile, setOpenMobile, width, isResizing } =
    useSidebar();
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "bg-sidebar text-sidebar-foreground flex h-full flex-col relative",
          className,
        )}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          width: state === "expanded" ? `${width}px` : "0px",
      ...
      <div
        ...
        style={{
          width: state === "expanded" ? `${width}px` : SIDEBAR_WIDTH_ICON,
          left:
